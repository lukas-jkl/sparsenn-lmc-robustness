# Code adapted from https://github.com/google-research/wide-sparse-nets
...
def make_smask_for_layer(num_W_layer, nwtf_layer, tensor_dims_layer, io_only):
    ...
    io_only (bool) - if True, only the io dims (the first two dims of a conv layer) are sparsified,
        ...
        Returns
        smask_for_layer (torch tensor, bool) - a mask for the layer with boolean values, "True" indicating that the
                                                corresponding weight will be frozen (set to zero).
    """
    if io_only:
    ...
    inds_1d_to_freeze = torch.randperm(num_W_layer)[:nwtf_layer]
...
def get_num_W_tot(model, ltypes=['Linear', 'Conv2d']):
    """
    Computes the number of weights in the model
    Operates by copying the model model, getting the tensor dims of each relevant layer (specified in ltypes),
     and summing over the products of dims of each tensor.
    """
...
def get_fanin(lkey, dims, connectivity):
    ...
    for layer of type fc (fully-connected) or conv (convolutional).
    """
    if 'conv' in lkey or 'downsample' in lkey or 'shortcut' in lkey:
        fan_in = dims[1] * dims[2] * dims[3] * connectivity  # for conv layer
    elif 'fc' in lkey or 'cl' in lkey or 'linear' in lkey or "layers_" in lkey or lkey == '0' or lkey == '1':
        fan_in = dims[1] * connectivity  # for fc layer
    else:
        raise Exception('<!> Can not compute fan-in - unknown layer type.')
...
def prep_model_and_smask(model, model_with_target_width, ltypes=['Linear', 'Conv2d']):
    """
    ...
    num_W_tot_base = get_num_W_tot(model_with_target_width, ltypes)
    ...
    num_wtf = get_nwtf(num_W_to_freeze_tot, num_W, tensor_dims, lnames_sorted, io_only)
    ...
    for lind in range(num_layers_to_sparsify):
        ...
        lnwtf = num_wtf[lind]
        lname_for_statedict = get_lname_for_statedict(
            lname)  # converting layer name to match layer key in model.state_dict()

        print(f'\n>>> Layer {lname}:')

        # ===== make smask for current layer
        print(f'-- Making smask ({lnwtf} weights to remove) ...')
        smask[lname] = make_smask_for_layer(lsize, lnwtf, ldims, io_only)
        num_1_in_smask = torch.sum(
            smask[lname])  # number of ones in smask (corresp to num weights to freeze) in given layer

        if io_only:
            assert False  # Removed support for this option
            # kernel_size = np.prod(ldims[-2:]) if len(ldims) == 4 else 1
            # assert kernel_size * num_1_in_smask == lntf, f"<!> smask is wrong! {kernel_size * num_1_in_smask}
            # not equal {lntf}"
        else:
        ...
        print('-- Applying smask ...')
        with torch.no_grad():
            layer = model.state_dict()[lname_for_statedict]
            mask_tensor(layer, smask[lname], io_only)
...
def get_nwtf(nwtf_tot, num_W, tensor_dims, lnames_sorted, io_only):
    ...
    Distribute the total number of weights to freeze over model layers.
      Note: In case of io_only=True, it is in general not possible to match nwtf_tot exactly, because weights
             are removed in packs of kernel_size (for most conv layers, kernel_size=9). Therefore, there is going to be
             a mismatch, num_W_tot will deviate from num_W_tot_base by up to kernel_size.
    Parameters
    ...
    # the bins of the staggered sparsification: array of max. num. of weights that can be frozen within the given layer
    # before the next-smaller layer gets involved into sparsification
    ...
    try:
        _, lim_ind = find_ge(ntf_lims, nwtf_tot)
    except ValueError:
        warnings.warn("Could not find bin for nwtf_tot -> using all layers, smaller layers will be bottlenecked!!!")
        lim_ind = len(ntf_lims)
    num_layers_to_sparsify = lim_ind + 1

    # base fill: chunks of num. weights that are frozen in each involved layer until all involved layers
    ...
    if io_only:
        ...
        assert sum(
            nwtf) + remainder == nwtf_tot, f"<!> nwtf+remainder ({sum(nwtf) + remainder}) not as expected ({nwtf_tot})!"
    else:
